...
const LATEST_PUBLISHED_VERSION = "0.3.1";
...
async function validateProjectOnboarding(root, errors) {
  ...
  if (PROJECT_RELEASE_STATE === "candidate") {
    ...
    for (const relative of candidateOnboardingFiles) {
      ...
      if (/unreleased|not a published\s+release|does not include these candidate changes|not part of the v0\.3\.2 installation|only in reviewed source|尚未发布|尚未包含|不包含在下文安装|不包含这些候选改动/iu.test(text)) {
    ...
    if (/unreleased|source candidate|latest published release remains|no v0\.3\.2 tag/iu.test(currentChangelog)) {
    ...
    if (!/The checked-in metadata describes 0\.3\.2 Release, dated \d{4}-\d{2}-\d{2}\./u.test(checklist)
        || /The checked-in state is a 0\.3\.2 source candidate|is intentionally unset/u.test(checklist)) {
    ...
    const checklistDate = checklist.match(/The checked-in metadata describes 0\.3\.2 Release, dated (\d{4}-\d{2}-\d{2})\./u)?.[1];
