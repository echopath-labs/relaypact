...
fallbacks. These experimental routes were introduced in v0.3.0.
...
The Host must select one exact model ID for every task. Other builds require
...
copy credentials, change persistent desktop settings or reuse another edition's
login. It binds the Host-selected model for the invocation with the native
`--model` argument and never supplies a fallback model. An open desktop
conversation may still have a separate per-session choice.
...
node <skill-directory>/scripts/relaypact.mjs doctor
  --route codex-workbuddy --edition mainland --model <model-id>

node <skill-directory>/scripts/relaypact.mjs doctor
  --route codex-workbuddy-ai --edition international --model <model-id>
```

Without `--model`, doctor checks only installation and native configuration
...
node <skill-directory>/scripts/relaypact.mjs run-workbuddy
  --edition international --model <model-id> --envelope <task-envelope.json> --read-only

node <skill-directory>/scripts/relaypact.mjs run-workbuddy
  --edition mainland --model <model-id> --envelope <task-envelope.json>
```
...
from returned evidence. `modelBinding` records the exact Host argument,
successful help preflight and disabled fallback. `modelObservation` remains
unavailable unless the native terminal result independently reports a model; a
binding is not proof of provider use or price.
