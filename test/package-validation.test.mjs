...
test("public package rejects published-install tag verification and metric guidance drift", async () => {
  ...
  for (const relative of [
    ...
    await writeFile(
      ...
      (await readFile(target, "utf8"))
        .replaceAll("git clone --branch v0.3.6", "git clone --branch release")
        .replaceAll("v0.3.6^{}", "v0.3.6")
  ...
  await writeFile(
    readme,
    (await readFile(readme, "utf8")).replace("Release target: **v0.3.6**", "Latest published release: **v0.1.1**")
  );
  const errors = await validatePackage(root);
  assert(errors.some((item) => item.includes("README.md must include \"git clone --branch v0.3.6\"")));
  assert(errors.some((item) => item.startsWith("README.md must include") && item.includes("git -C relaypact-v0.3.6 rev-parse") && item.includes("v0.3.6^{}")));
  assert(errors.some((item) => item.includes("README.md must include \"Release target: **v0.3.6**\"")));
...
async function copyCandidatePackage(t) {
  ...
  for (const file of ["README.md", "README.zh-CN.md", "docs/agent-quickstart.md", "docs/agent-quickstart.zh-CN.md", "docs/manual-configuration.md"]) {
    ...
    const text = (await readFile(target, "utf8"))
      .replaceAll("0.3.6", "0.3.5")
      .replace("Release target: **v0.3.5**", "Latest published release: **v0.3.5**")
      .replace("安装目标版本：**v0.3.5**", "最新已发布版本：**v0.3.5**");
    await writeFile(target, text + '\n0.3.6 Release source candidate\n`v0.3.6` is not released\n`v0.3.6` 尚未发布\n');
  }
  const changelog = path.join(root, "CHANGELOG.md");
  await writeFile(changelog, (await readFile(changelog, "utf8")).replace("## [0.3.6] - 2026-09-24 - Release", "## [0.3.6] - Unreleased - Release"));
...
test("candidate rejects premature release claims and installation in either language", async (t) => {
  ...
  for (const [file, claim] of [
    ["README.md", "Latest published release: **v0.3.6**"],
    ["README.zh-CN.md", "最新已发布版本：**v0.3.6**"]
  ]) {
    const target = path.join(root, file);
    await writeFile(target, `${await readFile(target, "utf8")}\n${claim}\ngit clone --branch v0.3.6\n`);
  }
  const errors = await validateCandidate(root);
  for (const file of ["README.md", "README.zh-CN.md"]) {
    assert(errors.some((item) => item.startsWith(`${file} must not include`) && item.includes("v0.3.6")));
...
test("candidate rejects stale versioned status and missing unreleased changelog", async (t) => {
  ...
  await writeFile(guide, (await readFile(guide, "utf8")).replace("0.3.6 Release source candidate", "0.3.5 Release source candidate"));
  const changelog = path.join(root, "CHANGELOG.md");
  await writeFile(changelog, (await readFile(changelog, "utf8")).replace("## [0.3.6] - Unreleased - Release", "## [0.3.6] - 2026-01-01 - Release"));
  const errors = await validateCandidate(root);
  assert(errors.some((item) => item.startsWith("docs/agent-quickstart.zh-CN.md must include") && item.includes("0.3.6 Release source candidate")));
...
test("versioned documentation requires complete install identity without publication claims", async (t) => {
  const root = await copyCurrentPublicPackage();
  t.after(() => rm(root, { recursive: true, force: true }));
  const comparisonLink = "[0.3.6]: https://github.com/echopath-labs/relaypact/compare/v0.3.5...v0.3.6";
  ...
  assert.deepEqual(await validatePackage(root), []);
  for (const invalidLink of ["", comparisonLink.replace("v0.3.5...", "v0.1.1...")]) {
    await writeFile(linkChangelog, `${datedChangelog}\n${invalidLink}\n`);
    assert((await validatePackage(root)).some((item) => item.startsWith("CHANGELOG.md must include") && item.includes("compare/v0.3.5...v0.3.6")));
  }
  await writeFile(linkChangelog, `${datedChangelog}\n${comparisonLink}\n`);
  for (const [file, claim] of [
  ...
  for (const file of ["README.md", "README.zh-CN.md", "docs/agent-quickstart.md", "docs/agent-quickstart.zh-CN.md", "docs/manual-configuration.md"]) {
    const target = path.join(root, file);
    const valid = await readFile(target, "utf8");
    const releaseUrl = "https://github.com/echopath-labs/relaypact/releases/tag/v0.3.6";
    const sections = valid.split(/(?=^#{1,6} )/mu);
    for (let index = 0; index < sections.length; index += 1) {
      if (!sections[index].includes("git clone --branch v0.3.6")) continue;
  ...
  for (const [file, from, to, expected] of [
    ["CHANGELOG.md", "## [0.3.6] - 2026-09-24 - Release", "## [0.3.6] - 2026-09-24 - Release\n\nThis is an unreleased source candidate.", "CHANGELOG.md must not retain candidate-only status"],
    ["RELEASING.md", "The checked-in metadata describes 0.3.6 Release, dated 2026-09-24.", "The checked-in state is a 0.3.6 source candidate. The release date is intentionally unset.", "RELEASING.md must describe the dated versioned current state."]
  ]) {
    const target = path.join(root, file);
    const valid = await readFile(target, "utf8");
  ...
  await writeFile(checklist, validChecklist.replace("dated 2026-09-24.", "dated 2026-09-15."));
  ...
  for (const [from, to, expected] of [
    ['p.version!=="0.3.6"', 'p.version!=="0.3.5"', 'if(p.version'],
    ['cd relaypact-v0.3.6', 'cd relaypact-v0.3.5', 'cd relaypact-v0.3.6'],
    ["git -C relaypact-v0.3.6 rev-parse 'v0.3.6^{}'", "git -C relaypact-v0.3.5 rev-parse 'v0.3.6^{}'", "git -C relaypact-v0.3.6 rev-parse 'v0.3.6^{}'"]
  ]) {
  ...
  await writeFile(chinese, (await readFile(chinese, "utf8")).replace("安装目标版本：**v0.3.6**", "最新已发布版本：**v0.3.6**"));
  const errors = await validatePackage(root);
  assert(errors.some((item) => item.includes("dated Release heading")));
  assert(errors.some((item) => item.startsWith("README.zh-CN.md must not include") && item.includes("v0.3.6")));
});
