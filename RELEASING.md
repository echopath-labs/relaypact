...
This checklist prepares a human-authorized `0.3.4` GitHub Release. It
...
## 0.3.4 release-time documentation closeout

The checked-in metadata describes 0.3.4 Release, dated 2026-09-21.
...
4. Prepare one scoped versioned-documentation commit:
   - Set PROJECT_RELEASE_STATE in scripts/validate-package.mjs to versioned;
     set PROJECT_VERSION to 0.3.4 and the previous published version at 0.3.3.
   - Update README.md, README.zh-CN.md, docs/agent-quickstart.md,
     ...
     v0.3.4 as the installation target, require a visible official GitHub Release
     before running install commands, and verify the peeled
     v0.3.4^{} commit. Remove candidate-only statements and the guidance caveat
     that applies to the older installation; retain explicitly historical notes.
   - Date the 0.3.4 changelog entry and add its comparison link.
   - Versioned documentation is not a publication announcement. Do not label
     v0.3.4 as latest published before the remote tag and Release exist.
     If publication fails after merge, installation must stop at the Release
     availability precondition; the previous v0.3.3 remains the fallback.
...
6. Under the explicit remote authorization, create the annotated v0.3.4 tag at
...
Describe 0.3.4 as an ordinary GitHub Release for practical Host-supervised
