...
resolves the selected executable to an absolute real path. For shebang launchers,
it also resolves the interpreter and invokes that absolute interpreter directly.
The signed fingerprint binds the launcher, interpreter, and fixed argument
prefix. Correction verifies that complete launch identity and preserves the
original read-only or write authority before the handle can be reused. Public
review and terminal archives retain only bounded evidence and never the raw
handle.
