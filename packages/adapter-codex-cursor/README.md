...
For zero-write direct execution, `allowedPaths: []` also requires an empty
`validation` list. Repository validation commands are refused before launch
because they can write caches or coverage; run them in an external read-only or
