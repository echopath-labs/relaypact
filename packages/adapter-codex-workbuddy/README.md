...
Every WorkBuddy read-only invocation, whether explicitly selected or derived
from empty `allowedPaths`, also requires `validation: []`; direct repository
validation commands are refused before native launch because they can write
caches or coverage. Run those checks in an external read-only or disposable
environment.
